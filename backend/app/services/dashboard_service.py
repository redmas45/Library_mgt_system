"""
Dashboard service — aggregates analytics for the admin dashboard.
"""
...
def get_dashboard(db: Session) -> DashboardResponse:
    ...
    # Top lists
    most_borrowed = [
        BookStat(**b) for b in get_most_borrowed_books(db, limit=10)
    ]
    most_searched = [
        BookStat(**b) for b in get_most_searched_books(db, limit=10)
    ]
    top_users_data = [
        UserStat(**u) for u in get_top_users(db, limit=10)
    ]
