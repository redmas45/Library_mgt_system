...
class SearchService:
    ...
    def search(
        self, db: Session, user: User, query: str,
        top_k: int = 5, book_id: Optional[int] = None,
    ) -> SearchResponse:
        raw_results = self.vector_store.search(query=query, top_k=top_k, book_id=book_id)

        # Fallback: if no vector hits, try title/author matches from the DB
        if not raw_results:
            ...
            for book in fallback_books:
                ...
                raw_results.append({
                    ...
                    "chunk_text": snippet, "page_number": None,
        ...
        create_interaction(
            ...
            interaction_type="search", query=query,
            response=f"Found {len(raw_results)} results", book_id=book_id,
        )

        results = [
            SearchResult(
                book_id=r["book_id"], book_title=r["book_title"],
                chunk_text=r["chunk_text"], relevance_score=r["relevance_score"],
