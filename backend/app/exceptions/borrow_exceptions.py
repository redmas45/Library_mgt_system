"""
Borrow-related custom exceptions.
"""
...
class AlreadyBorrowedError(HTTPException):
    def __init__(self, book_id: int):
        super().__init__(
            ...
            detail=f"You already have an active borrow for book ID {book_id}",
        )
...
class BookAlreadyReturnedError(HTTPException):
    def __init__(self, record_id: int):
        super().__init__(
            ...
            detail=f"Borrow record {record_id} has already been returned",
        )
...
class MaxBorrowLimitError(HTTPException):
    def __init__(self, limit: int = 5):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Maximum borrow limit of {limit} books reached",
        )
